...
class WebSocketRails.Channel
  ...
  constructor: (@name, @_dispatcher, @is_private = false, @on_success, @on_failure) ->
    ...
    @_queue = []
    if @is_private
      event_name = @_dispatcher.namespace + '.subscribe_private'
    else
      event_name = @_dispatcher.namespace + '.subscribe'
  ...
  destroy: ->
    if @connection_id == @_dispatcher._conn?.connection_id
      event_name = @_dispatcher.namespace + '.unsubscribe'
  ...
  dispatch: (event_name, message) ->
    if event_name == @_dispatcher.namespace + 'channel_token'
      @connection_id = @_dispatcher._conn?.connection_id
